...
const Bornforthis = {
  ...
  setWelcome_info: async () => {
    ...
    try {
      if (!ipStore) {
        const response = await fetch(`https://ip.bloggersht.com.cn/Local`);
  ...
  showWelcome: (ipStore) => {
    ...
    let dist = Bornforthis.getDistance(120.721097,31.275441,ipStore.data.lng, ipStore.data.lat),
      address,
      welcome_info;
    //根据国家、省份、城市信息自定义欢迎语
    //海外地区不支持省份及城市信息
    ...
    🗺️您距离 <strong>落叶不随</strong> 约有 <strong>${dist}</strong> 公里！<br>
    当前IP地址为：<br>
